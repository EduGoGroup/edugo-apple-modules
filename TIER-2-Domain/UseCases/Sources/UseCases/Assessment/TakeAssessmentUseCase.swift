...
public enum AssessmentState: String, Sendable, Equatable, Codable {
...
public enum AssessmentStateError: Error, Sendable, Equatable {
    case invalidTransition(from: AssessmentState, to: AssessmentState)
    case cannotSubmitFromState(AssessmentState)
    case cannotStartFromState(AssessmentState)
    case attemptExpired
...
extension AssessmentStateError: LocalizedError {
...
public actor TakeAssessmentUseCase {
    ...
    private var currentState: AssessmentState = .idle
    ...
    public func loadAssessment(input: TakeAssessmentInput) async throws -> Assessment {
        guard currentState == .idle || currentState == .ready else {
            throw AssessmentStateError.invalidTransition(from: currentState, to: .loading)
        }
    ...
    public func startAttempt() async throws -> UUID {
        guard currentState == .ready else {
            throw AssessmentStateError.cannotStartFromState(currentState)
        }

        guard let assessment = currentAssessment else {
            throw AssessmentStateError.assessmentNotLoaded
        }
    ...
    public func saveAnswer(
        ...
        guard currentState == .inProgress else {
            throw AssessmentStateError.noAttemptInProgress
        }
        ...
        if let assessment = currentAssessment,
           ...
           attempt.elapsedSeconds > timeLimit {
            throw AssessmentStateError.attemptExpired
        }
    ...
    public func submitAttempt() async throws -> AttemptResult {
        guard currentState == .inProgress else {
            throw AssessmentStateError.cannotSubmitFromState(currentState)
        }
        ...
        if submittedIdempotencyKeys.contains(attempt.idempotencyKey) {
            throw AssessmentStateError.duplicateSubmission
        }
        ...
        if missingCount > 0 {
            throw AssessmentStateError.incompleteAnswers(missing: missingCount)
        }
    ...
    private func transitionTo(_ newState: AssessmentState) async throws {
        let validTransitions: [AssessmentState: Set<AssessmentState>] = [
            .idle: [.loading],
            .loading: [.ready, .idle],
        ...
        guard let allowed = validTransitions[currentState],
            ...
            throw AssessmentStateError.invalidTransition(from: currentState, to: newState)
        }
