...
```typescript
// PSEUDO-CÓDIGO: Variables conceptuales para ilustrar el flujo
  ...
  try {
    while (cycle <= config.max_retries) {
      ...
      if (decisionData.decision === "SOFT_RETRY" && cycle <= config.max_retries) {
        ...
        // Validar físicamente que se aplicaron correcciones
        if (correctionData.files_modified && correctionData.files_modified.length > 0) {
          ...
          try {
            ...
            await logStep(`Validado físicamente: ${firstFile}`)
          } catch (e) {
...
try {
  ...
  console.error(`\n❌ ERROR CRÍTICO NO MANEJADO: ${globalError.message}`)
...
**IMPORTANTE**: Este try-catch global es la última línea de defensa. Cada fase DEBE tener su propio manejo de errores, pero este catch global previene que errores inesperados dejen el sistema en estado inconsistente.
