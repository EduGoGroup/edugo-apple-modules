...
extension Task where Success: Sendable, Failure == any Error {
    /// Ejecuta una operación con timeout.
    ///
    /// Extension en Task para sintaxis más fluida.
    ///
    /// - Parameters:
    ///   - timeout: Duración máxima
    ///   - operation: Operación a ejecutar
    ...
    ///
    /// ```swift
    /// let result = try await Task.withTimeout(.seconds(30)) {
    ///     try await fetchData()
    /// }
    /// ```
    public static func withTimeout(
        ...
        try await LocalPersistence.withTimeout(timeout, operation: operation)
    }
}
